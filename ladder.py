...
if __name__ == '__main__':
    ...
    ss_sampler = CategoriesSampler(ssdata.slabel, 100,
                                      args.train_way, args.meta_size+args.shot + args.query)
    ...
    for epoch in range(args.start_epoch, args.max_epoch + 1):
        ...
        for i, batch in enumerate(ss_loader, 1):
            ...
            data_shot, data_shot2, data_query = data[:m], data[m+p:m+2*p], data[m + 2*p:m + 2*p + q]
            ...
            loss =  F.cross_entropy(euclidean_metric(feats, proto2), label) + lam * F.kl_div(F.log_softmax(ulogits1[agree], dim=1), F.softmax(ulogits2[agree], dim=1)) #+ lam_labs*F.cross_entropy(logits2, label) + lam * F.kl_div(F.log_softmax(logits, dim=1), F.softmax(logits2, dim=1))
